    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-12">
          ...
          <div className="hidden md:flex items-center justify-center space-x-8 flex-1">
            {navItems.map((item) => (
              <motion.a
                key={item.href}
                href={item.href}
                className="text-[#2606AA] hover:text-[#ff1f53] font-medium relative group"
          ...
          {/* Mobile Menu Button */}
          <button 
            className="md:hidden text-[#2606AA] p-2"
              ...
              <div className="flex flex-col space-y-4 bg-white/90 backdrop-blur-sm rounded-xl p-4 border border-[#2606AA]/10">
                {navItems.map((item) => (
                  <a
                    key={item.href}
                    href={item.href}
                    className="text-[#2606AA] hover:text-[#ff1f53] font-medium px-4 py-2 rounded-lg hover:bg-[#2606AA]/5"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {item.label}
                  </a>
                ))}
                <button className="w-full px-4 py-2 bg-[#2606AA] text-white rounded-lg font-bold shadow-comic">
                  Register Now
                </button>
              </div>
            </motion.div>
